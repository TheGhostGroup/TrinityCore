...
#undef LOAD_DB2
    ...
    for (NamesProfanityEntry const* namesProfanity : sNamesProfanityStore)
    {
        ...
        if (namesProfanity->Language != -1)
            _nameValidators[namesProfanity->Language].emplace_back(namesProfanity->Name, Trinity::regex::icase | Trinity::regex::optimize);
        else
        {
            for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
            {
                ...
                _nameValidators[i].emplace_back(namesProfanity->Name, Trinity::regex::icase | Trinity::regex::optimize);
            }
        }
    }

    for (NamesReservedEntry const* namesReserved : sNamesReservedStore)
        _nameValidators[TOTAL_LOCALES].emplace_back(namesReserved->Name, Trinity::regex::icase | Trinity::regex::optimize);
    ...
    for (NamesReservedLocaleEntry const* namesReserved : sNamesReservedLocaleStore)
    {
        ...
        for (uint32 i = 0; i < TOTAL_LOCALES; ++i)
        {
            ...
            if (namesReserved->LocaleMask & (1 << i))
                _nameValidators[i].emplace_back(namesReserved->Name, Trinity::regex::icase | Trinity::regex::optimize);
        }
    }
